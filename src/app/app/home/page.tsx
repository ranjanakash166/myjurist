            ...
            {documentFeatures.map((feature, index) => {
              ...
              return (
                <Link key={index} href={feature.href} className="group">
                  <Card className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/50 dark:to-teal-950/50 border-emerald-200 dark:border-emerald-800/50 hover:scale-[1.02] hover:shadow-xl transition-all duration-300 cursor-pointer h-full">
                    <CardHeader className="pb-4">
                      <div className="flex items-start justify-between mb-4">
                        <div className="p-3 rounded-lg bg-emerald-600 dark:bg-emerald-700 group-hover:scale-110 transition-transform duration-300">
                          <Icon className="h-6 w-6 text-white" />
                        </div>
                        <ArrowRight className="h-5 w-5 text-emerald-600 dark:text-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {researchFeatures.map((feature, index) => {
              ...
              return (
                <Link key={index} href={feature.href} className="group">
                  <Card className="bg-gradient-to-br from-slate-50 to-gray-50 dark:from-slate-900/50 dark:to-gray-900/50 border-slate-200 dark:border-slate-800/50 hover:scale-[1.02] hover:shadow-xl transition-all duration-300 cursor-pointer h-full">
                    <CardHeader className="pb-4">
                      <div className="flex items-start justify-between mb-4">
                        <div className="p-3 rounded-lg bg-slate-600 dark:bg-slate-700 group-hover:scale-110 transition-transform duration-300">
                          <Icon className="h-6 w-6 text-white" />
                        </div>
                        <ArrowRight className="h-5 w-5 text-slate-600 dark:text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      </div>
                      <CardTitle className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                        {feature.title}
                      </CardTitle>
