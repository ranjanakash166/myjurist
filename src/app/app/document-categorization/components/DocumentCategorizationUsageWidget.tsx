...
function findCategorizationFeature(features: FeatureUsage[]): FeatureUsage | undefined {
  return features.find(
    (f) =>
      f.display_key.toLowerCase().includes("categor") ||
      f.display_name.toLowerCase().includes("categorization") ||
      f.display_name.toLowerCase().includes("document categor")
  );
}

export default function DocumentCategorizationUsageWidget() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setFeature(findCategorizationFeature(usage.features) ?? null);
      } catch {
