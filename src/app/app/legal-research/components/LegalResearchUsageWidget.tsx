...
function findLegalResearchFeature(features: FeatureUsage[]): FeatureUsage | undefined {
  return features.find(
    (f) =>
      f.display_key.toLowerCase().includes("legal") ||
      f.display_name.toLowerCase().includes("legal research") ||
      f.display_name.toLowerCase().includes("legal search")
  );
}

export default function LegalResearchUsageWidget() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setFeature(findLegalResearchFeature(usage.features) ?? null);
        }
      } catch {
  ...
  if (!feature) {
    return (
      <div className="rounded-lg bg-slate-100 p-3 space-y-2">
        <p className="text-xs text-slate-500">Legal searches this month</p>
