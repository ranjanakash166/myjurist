...
function findTimelineFeature(features: FeatureUsage[]): FeatureUsage | undefined {
  return features.find(
    (f) =>
      f.display_key.toLowerCase().includes("timeline") ||
      f.display_name.toLowerCase().includes("timeline")
  );
}

export default function TimelineExtractorUsageWidget() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setFeature(findTimelineFeature(usage.features) ?? null);
      } catch {
